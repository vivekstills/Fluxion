...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    const resize = () => {
        if (!rendererRef.current || !ctn) return;
    ...
    if (mouseReact) currentCtn.addEventListener('mousemove', handleMouseMove as EventListener);

    return () => {
      cancelAnimationFrame(rafRef.current);
      if(currentCtn) {
          resizeObserver.unobserve(currentCtn);
          if (mouseReact) currentCtn.removeEventListener('mousemove', handleMouseMove as EventListener);
          if (gl.canvas.parentElement === currentCtn) currentCtn.removeChild(gl.canvas);
      }
